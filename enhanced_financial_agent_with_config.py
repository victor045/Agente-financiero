"""
Enhanced Financial Agent - Con Configuración Externa.
Versión que usa configuración externa para las fuentes de datos.
"""
...
try:
    from config_data_sources import DataSourceConfig, get_config, create_custom_config
except ImportError:
    print("⚠️  No se pudo importar config_data_sources.py")
...
class FinancialAgentConfig:
    ...
    # Configuración de datos - Usar configuración externa
    data_directory: str = None  # Se obtiene de DataSourceConfig
...
class FinancialDataProcessor:
    ...
    def __init__(self, config: FinancialAgentConfig, data_source_config: DataSourceConfig):
        self.config = config
        self.data_source_config = data_source_config
        self.data_directory = data_source_config.get_data_directory_path()
        self.data = {}
...
###################
# Enhanced Financial Agent with Config
###################

class EnhancedFinancialAgentWithConfig:
    """Agente financiero con configuración externa."""
    
    def __init__(self, data_source_config: DataSourceConfig = None, agent_config: FinancialAgentConfig = None):
        self.agent_config = agent_config or FinancialAgentConfig()
        self.data_source_config = data_source_config or get_config("default")
        self.data_processor = FinancialDataProcessor(self.agent_config, self.data_source_config)
        self.question_interpreter = QuestionInterpreter()
        self.response_formatter = ResponseFormatter()
        self.visualizer = SimpleGraphVisualizer() if self.agent_config.enable_graph_visualization else None
        self.current_state = None
        self.execution_history = []
        
        # Mostrar información de fuentes de datos
        self.data_source_config.show_data_info()
        
        # Mostrar grafo inicial si está habilitada la visualización
        if self.visualizer and self.agent_config.enable_graph_visualization:
...
def main():
    """Función principal del agente con configuración externa."""
    print("🎯 ENHANCED FINANCIAL AGENT - CON CONFIGURACIÓN EXTERNA")
    print("=" * 60)
    ...
    print("   ✅ Configuración externa de fuentes de datos")
    print("=" * 60)
    
    # Crear agente con configuración por defecto
    agent = EnhancedFinancialAgentWithConfig()
