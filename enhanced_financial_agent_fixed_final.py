"""
Enhanced Financial Agent - Versión Final Corregida.
Versión que maneja correctamente las preguntas específicas y no se queda en loops.
"""
...
class FinancialAgentState:
    ...
    def __init__(self):
        ...
        self.fecha_filtro: Optional[str] = None
...
class FinancialDataProcessor:
    ...
    def load_all_data(self) -> Dict[str, pd.DataFrame]:
        ...
        print(f"📁 Desde: {self.data_directory}")
        ...
        if fecha_filtro:
            ...
            if fecha_col:
                try:
                    ...
                    # Filtrar por mes
                    if fecha_filtro == "mayo":
                        df = df[df[fecha_col].dt.month == 5]
                    elif fecha_filtro == "junio":
                        df = df[df[fecha_col].dt.month == 6]
                    elif fecha_filtro == "julio":
                        df = df[df[fecha_col].dt.month == 7]
                    elif fecha_filtro == "agosto":
                        df = df[df[fecha_col].dt.month == 8]
                    elif fecha_filtro == "septiembre":
                        df = df[df[fecha_col].dt.month == 9]
                    elif fecha_filtro == "octubre":
                        df = df[df[fecha_col].dt.month == 10]
                    elif fecha_filtro == "noviembre":
                        df = df[df[fecha_col].dt.month == 11]
                    elif fecha_filtro == "diciembre":
                        df = df[df[fecha_col].dt.month == 12]
                    elif fecha_filtro == "enero":
                        df = df[df[fecha_col].dt.month == 1]
                    elif fecha_filtro == "febrero":
                        df = df[df[fecha_col].dt.month == 2]
                    elif fecha_filtro == "marzo":
                        df = df[df[fecha_col].dt.month == 3]
                    elif fecha_filtro == "abril":
                        df = df[df[fecha_col].dt.month == 4]
                    
                    analysis['fecha_filtro'] = fecha_filtro
                    analysis['registros_filtrados'] = len(df)
                    print(f"📅 Filtro aplicado: {fecha_filtro} - {len(df)} registros")
...
"""
            ...
            fecha = analysis_results.get('fecha_filtro', 'período especificado')
            return f"""
📊 Executive Summary
Total de facturas por cobrar en {fecha}: ${analysis_results['por_cobrar']:,.2f} MXN

📈 Detailed Analysis
- Total facturas por cobrar en {fecha}: ${analysis_results['por_cobrar']:,.2f} MXN
- Número de facturas por cobrar: {analysis_results.get('por_cobrar_count', 0)}
- Promedio por factura por cobrar: ${analysis_results.get('por_cobrar_promedio', 0):,.2f} MXN
- Factura por cobrar más alta: ${analysis_results.get('por_cobrar_max', 0):,.2f} MXN
- Registros analizados: {analysis_results.get('registros_filtrados', 0)}

🔍 Data Sources Used
- facturas.xlsx: Filtrado por {fecha} y tipo "Por cobrar"

💡 Key Insights
- Total de facturas por cobrar en {fecha}: ${analysis_results['por_cobrar']:,.2f} MXN
- Cantidad específica: ${analysis_results['por_cobrar']:,.2f} pesos mexicanos
- {analysis_results.get('por_cobrar_count', 0)} facturas por cobrar en {fecha}
"""
            ...
            return f"""
📊 Executive Summary
Total de facturas por pagar en {fecha}: ${analysis_results['por_pagar']:,.2f} MXN

📈 Detailed Analysis
- Total facturas por pagar en {fecha}: ${analysis_results['por_pagar']:,.2f} MXN
- Número de facturas por pagar: {analysis_results.get('por_pagar_count', 0)}
- Promedio por factura por pagar: ${analysis_results.get('por_pagar_promedio', 0):,.2f} MXN
- Factura por pagar más alta: ${analysis_results.get('por_pagar_max', 0):,.2f} MXN
- Registros analizados: {analysis_results.get('registros_filtrados', 0)}

🔍 Data Sources Used
- facturas.xlsx: Filtrado por {fecha} y tipo "Por pagar"

💡 Key Insights
- Total de facturas por pagar en {fecha}: ${analysis_results['por_pagar']:,.2f} MXN
- Cantidad específica: ${analysis_results['por_pagar']:,.2f} pesos mexicanos
- {analysis_results.get('por_pagar_count', 0)} facturas por pagar en {fecha}
"""
...
###################
# Enhanced Financial Agent Fixed Final
###################

class EnhancedFinancialAgentFixedFinal:
    """Agente financiero corregido final."""
    ...
    def process_question(self, question: str) -> str:
        ...
        try:
            ...
            if state.fecha_filtro:
                print(f"   📅 Filtro de fecha: {state.fecha_filtro}")
...
def main():
    """Función principal del agente corregido final."""
    print("🎯 ENHANCED FINANCIAL AGENT - VERSIÓN FINAL CORREGIDA")
    print("=" * 60)
    ...
    # Crear agente
    agent = EnhancedFinancialAgentFixedFinal()
