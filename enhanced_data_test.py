"""
Enhanced test script that properly handles the real Excel data structure.
"""
...
def load_and_analyze_real_data():
    """Load and analyze the real Excel files with proper column mapping."""
...
def analyze_financial_data_enhanced(all_data, summaries):
    """Analyze financial data with proper column mapping."""
    ...
    if 'facturas.xlsx' in all_data:
        ...
        amount_col = 'monto_mxn' if 'monto_mxn' in facturas_df.columns else 'monto'
        client_col = 'clienteproveedor' if 'clienteproveedor' in facturas_df.columns else 'cliente'
        date_col = 'fecha_de_emisión' if 'fecha_de_emisión' in facturas_df.columns else 'fecha'
        type_col = 'tipo' if 'tipo' in facturas_df.columns else None
        ...
        if type_col and type_col in facturas_df.columns:
        ...
        # Analyze by client
        if client_col and client_col in facturas_df.columns:
        ...
        if date_col and date_col in facturas_df.columns:
    ...
    if 'gastos_fijos.xlsx' in all_data:
        ...
        # Map to expected columns
        amount_col = 'monto_mxn' if 'monto_mxn' in gastos_df.columns else 'monto'
        category_col = 'gasto_fijo' if 'gasto_fijo' in gastos_df.columns else 'rubro'
        ...
        if category_col and category_col in gastos_df.columns:
    ...
    if 'Estado_cuenta.xlsx' in all_data:
        ...
        # Map to expected columns
        amount_col = 'monto_de_la_transacción_mxn' if 'monto_de_la_transacción_mxn' in estado_df.columns else 'monto'
        date_col = 'fecha' if 'fecha' in estado_df.columns else None
        desc_col = 'descripción_de_la_transacción' if 'descripción_de_la_transacción' in estado_df.columns else 'descripción'
        ...
        if date_col and date_col in estado_df.columns:
...
def simulate_detailed_responses(questions, analysis_results):
    ...
    for i, question in enumerate(questions[:5], 1):  # Show first 5 questions
        ...
        if "total de facturas" in question.lower() and 'facturas' in analysis_results:
            ...
            print(f"- facturas.xlsx: folio_de_factura, tipo, clienteproveedor, fecha_de_emisión, monto_mxn")
            ...
            print(f"🔍 Data Sources Used")
            print(f"- gastos_fijos.xlsx: gasto_fijo, monto_mxn, día_del_mes_para_hacer_pago")
            print(f"💡 Key Insights")
            ...
            print(f"🔍 Data Sources Used")
            print(f"- Estado_cuenta.xlsx: fecha, descripción_de_la_transacción, monto_de_la_transacción_mxn, saldo_mxn")
            print(f"💡 Key Insights")
            print(f"- Net cash flow: ${net_flow:,.2f}")
...
def main():
    """Main function to run the enhanced data analysis."""
    print("🔍 Enhanced Financial Data Analysis - Real Data Testing")
    print("=" * 60)
    ...
    try:
        ...
        # Analyze financial data with enhanced column mapping
        analysis_results = analyze_financial_data_enhanced(all_data, summaries)
